...
#     def test_successful_transcription(self, mock_pydub, mock_requests_get, mock_requests_post):
#         """
...
#         self.assertEqual(json.loads(file_obj.transcription_file.read().decode('utf-8')), self.mock_transcription_data)
...
#         self.assertIsNone(file_obj.transcription_file.name)
...
#         mock_transcript_content = json.dumps({"text": "A long audio transcript."})
#         self.audiobook_file.transcription_file.save("mock_transcript.json", ContentFile(mock_transcript_content))
